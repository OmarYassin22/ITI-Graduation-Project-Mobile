...
import { Text, Headline, Searchbar, useTheme, Button } from 'react-native-paper';
...
const STRIPE_PUBLISHABLE_KEY = 'pk_test_51PrP4TBp38FM06vYjqUSTOWFAdyY60sB8ibBsgfWBIbVReUBAauq8qRPyJR4TRdBgzyBY1SnxeRN23cmyQg4wu2Y00iBHn5Kvm';
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        Alert.alert('Processing', 'Payment successful! Updating your courses...');
        ...
        if (buyerUser) {
          ...
          Alert.alert('Success', 'Your courses have been added to your learning list!', [
            { text: 'OK', onPress: () => navigation.navigate('Mylearning') }
          ]);
        } else {
          console.error("No user document found for email:", buyerEmail);
          throw new Error(`User document not found for email: ${buyerEmail}. Please check your account details.`);
        }
      }
    } catch (error) {
      console.error('Payment or update error:', error);
      Alert.alert('Error', error.message || 'There was a problem processing your payment or updating your courses. Please try again.');
    } finally {
      setLoading(false);
    }
  };
